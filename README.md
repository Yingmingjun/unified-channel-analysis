...
Aggregate per-link data sufficient to reproduce every paper figure and
table out of the box — no raw measurements required.

| Included | Contents |
|---|---|
| `data/point_data/*.xlsx` (6 files) | Per-link point-data tables (N1, N3, U1, U3 at sub-THz + 6.75 GHz) — PL / DS / ASA / ASD per TX–RX pair, the machine-readable form of the paper's Tables U3 / N3 and their supplementary full versions. |
| `data/point_data/*_Results.csv` (4 files) | Per-link processing-comparison CSVs (NYU 142, NYU 7, USC 145, USC 7) consumed by the Bland-Altman, CDF, and PL-CI scripts. |
| `matlab/processing/*/Results/*_Results.csv` | Same per-link aggregate CSVs, kept adjacent to the processing scripts that emit them. |
| `matlab/processing/*/Results/*.mat` | Stripped MATLAB result structs containing **only** `config`, `params`, and the per-link `results` struct (PL / DS / AS summaries, location metadata). Typically 3–31 KB each. |

## What is explicitly **not** shipped

Raw or intermediate measurement data. If you want to re-run the raw
processing stage, you supply your own PDPs organized per
[`DATA_ORGANIZATION.md`](./DATA_ORGANIZATION.md); the figure/table
scripts run out of the box against the bundled aggregate data above.

| Excluded | Why |
|---|---|
| Raw / thresholded PDPs | Campaign-specific IP; you provide your own measurements. |
| `pas_store` in `Results/*.mat` | 99 %+ of the original `.mat` footprint; stripped before publishing. |
