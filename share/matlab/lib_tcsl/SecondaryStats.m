function [Sec_statTable,statTable]=SecondaryStats(statTable,elevPatternFile,aziPatternFile)
    ...
    for iloc = 1:Nlocs
        ...
        % Omni ASD
        [~,omni_asd(iloc)] = compute_angular_spread(MPC_AOD,MPCpowers,'deg','deg');
        % Omni ZSD
        [omni_zsd(iloc),~] = computeDSonMPC(MPC_ZOD+90,MPCpowers,mpc_threshold);
        % Omni ASA
        [~,omni_asa(iloc)] = compute_angular_spread(MPC_AOA,MPCpowers,'deg','deg');
        % Omni ZSA
        [omni_zsa(iloc),~] = computeDSonMPC(MPC_ZOA+90,MPCpowers,mpc_threshold);
        % Dir DS
        ...
        for j = 1:nAOA_SL
            ...
            if(~isempty(part_array))
                [~,lobe_asa_temp(j)] = compute_angular_spread(part_array(:,5),part_array(:,2),'deg','deg');
                [lobe_zsa_temp(j),~] = computeDSonMPC(part_array(:,6)+90,part_array(:,2),mpc_threshold);
            else
                %lobe_asa_temp(j)=[];
                %lobe_zsa_temp(j)=[];
                rmv_idx(j)=true;
        ...
        for j = 1:nAOD_SL
            ...
            part_array = MPCarray(SLmask,:);
            if(~isempty(part_array))
                [~,lobe_asd_temp(j)] = compute_angular_spread(part_array(:,3),part_array(:,2),'deg','deg');
                [lobe_zsd_temp(j),~] = computeDSonMPC(part_array(:,4)+90,part_array(:,2),mpc_threshold);
            else
                %lobe_asd_temp(j)=[];
                %lobe_zsd_temp(j)=[];
                rmv_idx(j)=true;
