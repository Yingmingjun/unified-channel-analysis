function [AllSPRefAngles,AllSPZRefAngles,AllSPZAltAngles,AllSPAltAngles,SP_idx,SP_pwr]=SubPathPwrDirsD(PAS_set,lobeCount,lobewidths,ends,starts,PAS_angles,PAS_powers,multipath_low_bound,SLT,aziPatternFile,elevPatternFile)
    ...
    for iLobe=1:lobeCount
        ...
        SLpowers=PAS_powers(idx);
        SLangles(SLangles==360)=0;
        [SLangles,sidx]=sort(SLangles,'ascend');
        SLpowers=SLpowers(sidx);
        ...
        [~,Ang_pos]=min(abs(SLpowers(SLangles==starts(iLobe))-SLT)-abs(azi));
        mpc1_RefAng=SLangles(SLangles==starts(iLobe))-abs(aziPatternFile(Ang_pos,1));
        mpc1_AltAng=SLangles(SLangles==starts(iLobe))-abs(aziPatternFile(Ang_pos,1));
        mpc1_pow=SLT;
        mpc2_ang=SLangles(SLangles==ends(iLobe))+abs(aziPatternFile(Ang_pos,1));
        mpc2_pow=SLT;
