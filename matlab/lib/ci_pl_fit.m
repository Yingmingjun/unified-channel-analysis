function [ple, sigma_sf, cfi_lo, cfi_hi, cfi_width] = ci_pl_fit(d, pl, freq_ghz, n_boot, seed)
% ci_pl_fit  Close-In path-loss fit with bootstrap 95% CFI on the PLE.
%
%   [ple, sigma_sf, cfi_lo, cfi_hi, cfi_width] = ci_pl_fit(d, pl, freq_ghz,
%                                                          n_boot, seed)
%
...
if numel(d) < 2
    ple = NaN; sigma_sf = NaN; cfi_lo = NaN; cfi_hi = NaN; cfi_width = NaN;
    return
end
