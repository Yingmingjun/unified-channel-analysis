...
function plot_cdf_group(metricName, n3_vals, u3_vals, n3_isLOS, n3_isNLOS, u3_isLOS, u3_isNLOS)
    % Plot LOS and NLOS in subplots with NYU, USC, and pooled curves.
    figure('Name', ['CDF: ' metricName], 'Position', [200, 200, 1000, 420]);
    
    % LOS
    subplot(1,2,1);
    hold on; grid on; box on;
    plot_cdf_with_band(n3_vals(n3_isLOS), [0 0.45 0.74], 'NYU');
    plot_cdf_with_band(u3_vals(u3_isLOS), [0.85 0.33 0.10], 'USC');
    plot_cdf_with_band([n3_vals(n3_isLOS); u3_vals(u3_isLOS)], [0.2 0.2 0.2], 'Pooled');
    title([metricName ' CDF (LOS)']);
    xlabel(metricName); ylabel('CDF');
    legend('NYU','NYU 95% band','USC','USC 95% band','Pooled','Pooled 95% band','Location','best');
    
    % NLOS
    subplot(1,2,2);
    hold on; grid on; box on;
    plot_cdf_with_band(n3_vals(n3_isNLOS), [0 0.45 0.74], 'NYU');
    plot_cdf_with_band(u3_vals(u3_isNLOS), [0.85 0.33 0.10], 'USC');
    plot_cdf_with_band([n3_vals(n3_isNLOS); u3_vals(u3_isNLOS)], [0.2 0.2 0.2], 'Pooled');
    title([metricName ' CDF (NLOS)']);
    xlabel(metricName); ylabel('CDF');
    legend('NYU','NYU 95% band','USC','USC 95% band','Pooled','Pooled 95% band','Location','best');
end

function plot_cdf_with_band(vals, color, labelName)
    vals = vals(isfinite(vals));
    if isempty(vals)
        return;
    end
    [f, x] = ecdf(vals);
    plot(x, f, 'LineWidth', 1.6, 'Color', color);
    % DKW band (uniform) for 95% confidence
    n = numel(vals);
    eps = sqrt(log(2/0.05) / (2*n));
    f_lo = max(0, f - eps);
    f_hi = min(1, f + eps);
    fill([x; flipud(x)], [f_lo; flipud(f_hi)], color, 'FaceAlpha', 0.12, 'EdgeColor', 'none');
end
...
function plot_ci_models(label, d, pl, isLOS, isNLOS, fGHz, d0)
    ...
    hold on; grid on; box on;
    [n_los, ci_los] = plot_ci_fit(d(valid & isLOS), pl(valid & isLOS), fGHz, d0, 'LOS', 'o', [0 0.45 0.74]);
    [n_nlos, ci_nlos] = plot_ci_fit(d(valid & isNLOS), pl(valid & isNLOS), fGHz, d0, 'NLOS', 's', [0.85 0.33 0.10]);
    title(label);
    ...
    fprintf('%s\n', label);
    if ~isempty(n_los)
        fprintf('  LOS  n = %.3f (95%% CI %.3f, %.3f)\n', n_los, ci_los(1), ci_los(2));
    end
    if ~isempty(n_nlos)
        fprintf('  NLOS n = %.3f (95%% CI %.3f, %.3f)\n', n_nlos, ci_nlos(1), ci_nlos(2));
    end
end

function [n, n_ci] = plot_ci_fit(d, pl, fGHz, d0, tag, marker, color)
    d = d(:);
    pl = pl(:);
    d = d(isfinite(d));
    pl = pl(isfinite(pl));
