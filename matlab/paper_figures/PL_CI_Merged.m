...
function generate_pl_fig(T, band, pooledFreqGHz, xlimM, ylimDB, titleStr, ...
    ...
    % Use plot() instead of scatter() so the marker MarkerSize (in points)
    % is honored both on the plot and inside the legend box. With scatter,
    % MATLAB renders legend symbols at a smaller fixed size that does not
    % scale with SizeData, leaving the legend markers visually too small.
    h_nyu_los  = plot(los_nyu.d_m,  los_nyu.pl_db,  'o', ...
                      'MarkerSize', 14, 'MarkerEdgeColor', cLOS,  ...
                      'LineWidth', 1.6, 'LineStyle', 'none');
    h_usc_los  = plot(los_usc.d_m,  los_usc.pl_db,  '*', ...
                      'MarkerSize', 14, 'MarkerEdgeColor', cLOS,  ...
                      'LineWidth', 1.6, 'LineStyle', 'none');
    h_nyu_nlos = plot(nlos_nyu.d_m, nlos_nyu.pl_db, 'd', ...
                      'MarkerSize', 14, 'MarkerEdgeColor', cNLOS, ...
                      'LineWidth', 1.6, 'LineStyle', 'none');
    h_usc_nlos = plot(nlos_usc.d_m, nlos_usc.pl_db, 's', ...
                      'MarkerSize', 14, 'MarkerEdgeColor', cNLOS, ...
                      'LineWidth', 1.6, 'LineStyle', 'none');
