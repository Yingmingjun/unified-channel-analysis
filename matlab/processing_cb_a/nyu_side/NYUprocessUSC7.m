...
patternFile = P.cb_a_usc_3d_pattern;
...
%Currently using proxies until USC pattern received.
elevPatternFile = importdata(P.cb_a_usc_eplane_7);
aziPatternFile = importdata(P.cb_a_usc_hplane_7);
...
RXAntGainCorr=3.7; %Since USC PDPs already calibrate out the antenna, 
...
TXAntGainCorr=3.7; % Antenna Gain
...
rootDir=dir(fullfile(nyuFormatDir, Adata_glob));
...
for iTR=1:nTR
    RXid=sscanf(TR_str(iTR),"NYUformat_PDP_RX%d");
    %RXid=iTR;
    TXid=1;
    ...
    % %%%% Run Thresholding over entire dataset
    % Note all USC PDPs are in linear scale
    allPDPsLin=[TRpdpSet{:,1}];
    allPDPsdB=pow2db(allPDPsLin);
    
    % 25 dB below pk threshold
    maxPDPs=max(allPDPsdB,[],1);
    thres_25blwPk=maxPDPs-thres_below_pk;

    % 5 dB abv noise threshold
    % since NYU took 5000 samples for finding noise in a dilated PDP
    % 5000 sa == 250 ns
    % USC's PDP is undilated and is 1000 ns. So we take 250 points from the end
    thres_noisePlus5=pow2db(mean(allPDPsLin(end-250:end,:)))+thres_abv_noise;

    DDThreshold=max(thres_25blwPk,thres_noisePlus5);

    allPDPsdB(allPDPsdB<DDThreshold)=multipath_low_bound;

    %Next, filter across all PDPs and discard all rows in TRpdpSet that
    %have no MPC peaks.
    selIdx=true(1,width(allPDPsdB));
    for iPDP=1:width(allPDPsdB)
        if(isempty(allPDPsdB(allPDPsdB(:,iPDP)>multipath_low_bound,iPDP)))
            selIdx(iPDP)=false;
        else
...
% The legacy reference CSVs live alongside the script directory; try both
% the repo point-data dir and the processing_cb_a/nyu_side/OriginalUSC-PointData fallback.
uscLOS_candidates = { ...
    fullfile(P.point_data, 'usc_microcellular_LOS_metrics7.csv'), ...
    fullfile(fileparts(mfilename('fullpath')), 'OriginalUSC-PointData', 'usc_microcellular_LOS_metrics7.csv') ...
};
uscNLOS_candidates = { ...
    fullfile(P.point_data, 'usc_microcellular_NLOS_metrics7.csv'), ...
    fullfile(fileparts(mfilename('fullpath')), 'OriginalUSC-PointData', 'usc_microcellular_NLOS_metrics7.csv') ...
};
uscLOS_path = '';  uscNLOS_path = '';
for kk = 1:numel(uscLOS_candidates)
    if exist(uscLOS_candidates{kk}, 'file'), uscLOS_path = uscLOS_candidates{kk}; break; end
end
for kk = 1:numel(uscNLOS_candidates)
    if exist(uscNLOS_candidates{kk}, 'file'), uscNLOS_path = uscNLOS_candidates{kk}; break; end
end
uscLOS = readtable(uscLOS_path);
uscNLOS = readtable(uscNLOS_path);
...
% Redirect auxiliary CSV outputs + the canonical U3 xlsx target into the
% unified point_data/ directory so paper-figure drivers pick them up.
outDir_cb_a = fileparts(P.cb_a_out_u3_7_xlsx);
if ~exist(outDir_cb_a, 'dir'), mkdir(outDir_cb_a); end
writetable(pl_table_LOS,  fullfile(outDir_cb_a, 'PL_comparison_LOS_7.csv'));
writetable(pl_table_NLOS, fullfile(outDir_cb_a, 'PL_comparison_NLOS_7.csv'));
% Canonical xlsx sink for downstream drivers (U3 = USC-cross-processed xlsx).
% The current script computes per-link PL; write a FinalTable-compatible sheet so
% the paper-figure drivers see fresh data.
try
    writetable(pl_table_LOS,  P.cb_a_out_u3_7_xlsx, 'Sheet', 'LOS');
