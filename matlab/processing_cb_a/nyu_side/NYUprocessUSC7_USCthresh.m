...
for iTR=1:nTR_local
    ...
    if any(validMask)
        ...
        PL_calc_dB(iTR) = -Prx_dBm + RXAntGainCorr; % Pt = 0 dBm
        %USC PDPs need a gain correction since antennas are not stepped in
        %HPBW steps but slightly lower
    end
end

% Load USC original results (LOS / NLOS) via repo point-data dir with fallback.
uscLOS_candidates = { ...
...
% Redirect auxiliary CSV outputs + canonical U3 xlsx target into the
% unified point_data/ directory.
outDir_cb_a = fileparts(P.cb_a_out_u3_7_xlsx);
if ~exist(outDir_cb_a, 'dir'), mkdir(outDir_cb_a); end
writetable(pl_table_LOS,  fullfile(outDir_cb_a, 'PL_comparison_LOS_7_USCthresh.csv'));
writetable(pl_table_NLOS, fullfile(outDir_cb_a, 'PL_comparison_NLOS_7_USCthresh.csv'));
try
    writetable(pl_table_LOS,  P.cb_a_out_u3_7_xlsx, 'Sheet', 'LOS_USCthresh');
    writetable(pl_table_NLOS, P.cb_a_out_u3_7_xlsx, 'Sheet', 'NLOS_USCthresh');
catch ME_xlsx
