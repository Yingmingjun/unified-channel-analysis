function [AllSPRefAngles,AllSPZRefAngles,AllSPZAltAngles,AllSPAltAngles,SP_idx,SP_pwr]=SubPathPwrDirs(PAS_set,lobeCount,lobewidths,ends,starts,PAS_angles,multipath_low_bound)
    ...
    for iLobe=1:lobeCount
        ...
        thres_noisePlus5=pow2db(mean(db2pow(SemiOmniPDP(end-250:end,:))))+5;
    
        %DDThreshold=max(thres_25blwPk,thres_noisePlus5);
        DDThreshold=thres_noisePlus5;
        SemiOmniPDP(SemiOmniPDP<DDThreshold)=multipath_low_bound;
        SemiOmniPDP=PDPdenoise(SemiOmniPDP,multipath_low_bound);
        
        [~,SeOmSPs]=findpeaks(SemiOmniPDP,"MinPeakDistance",4); %USC PDP undilated
        nSeOmSPs=length(SeOmSPs);

        IdxRng = bsxfun(@plus, SeOmSPs, -1:+2);
        IdxRng = mod(IdxRng,length(SemiOmniPDP));
        IdxRng(IdxRng==0) = 1;
        if (nSeOmSPs==1)
