function [Sec_statTable,statTable]=SecondaryStats_circD(statTable,elevPatternFile,aziPatternFile)
    ...
    for iloc = 1:Nlocs
        tempDlys=vertcat(statTable{iloc,16});%USC PDP is undilated
        ...
        pdp=[statTable{iloc,1}];
        Npdp=length(pdp);
        t=(0:Npdp-1)./1;% divide by 20 for NYU data. USC pdp is undilated. Divide by 1. 
        omni_ds(iloc) = rms_delay_spread_calc(pdp,t,-200);
        % Omni ASD
        ...
        % Here, log-normal operations are carried out correctly using
        % natural log
        % For 3GPP comparisons, the mean and std need to be calculated
        % using log10 instead of log. Say mean10=log10() and std10=log10().
        % To express those back  into linear domain, the correct conversion
        % formula is:
        % = exp(log(10)*mean10 + 0.5*(log(10)*sigma10)^2)
        Sec_statTable{iloc,14}=exp(log(10)*mean(log10(dir_ds(dir_ds>1e-2)))+(0.5*((log(10)*std(log10(dir_ds(dir_ds>1e-2))))^2)));
        Sec_statTable{iloc,15}=exp(log(10)*mean(log10(lobe_asa))+(0.5*(log(10)*(std(log10(lobe_asa)))^2)));
        Sec_statTable{iloc,16}=exp(log(10)*mean(log10(lobe_asd))+(0.5*(log(10)*(std(log10(lobe_asd)))^2)));
        Sec_statTable{iloc,17}=exp(log(10)*mean(log10(lobe_zsa))+(0.5*(log(10)*(std(log10(lobe_zsa)))^2)));
        Sec_statTable{iloc,18}=exp(log(10)*mean(log10(lobe_zsd))+(0.5*(log(10)*(std(log10(lobe_zsd)))^2)));
    end
end