...
%e.g. for AOD_PAS pass the AOD column number
    ...
    PAS(PAS(:,2)==0,2)=360;
    %For USC the TX PAS has negative angles. Those need to be replaced with
    % positive equivalents by mod
    % Basically need to map negative angles to within 360 degrees 
    PAS(PAS(:,2)<0,2)=mod(PAS(PAS(:,2)<0,2),360);
