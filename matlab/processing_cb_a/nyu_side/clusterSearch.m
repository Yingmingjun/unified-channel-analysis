...
function [TCs,TCstart,TCstop,TCxsDelay,Nsp,iraClXsDly,SPpwrs,ierClXsDly]=clusterSearch(PDP,multipath_low_bound,MTI)
    ...
    %USC Omni PDP needs a second level of DD thresholding as multipath
    %noise bound is too high for USC calibrated PDPs.
    % 25 dB below pk threshold
    % maxPDPval=max(PDP);
    % thres_25blwPk=maxPDPval-25;
    ...
    thres_noisePlus5=pow2db(mean(db2pow(PDP(end-250:end,:))))+5;

    %DDThreshold=max(thres_25blwPk,thres_noisePlus5);
    DDThreshold=thres_noisePlus5;
    PDP(PDP<DDThreshold)=multipath_low_bound;
    ...
    %USC undilated data doesn't need minor spike cleaning
    ...
    if (Nmax==1)
        TCxsDelay=0;
        [~,Allpkidcs]=findpeaks(PDP,"MinPeakDistance",4); %USC PDPs are undilated
        pkidcs=Allpkidcs((Allpkidcs>=TCstart)&(Allpkidcs<=TCstop));
        Nsp=length(pkidcs);
        iraClXsDly={(pkidcs-pkidcs(1))}; 
        ierClXsDly=[];
        IdxRng = bsxfun(@plus, pkidcs, -1:+2);
        %USC PDPs are undilated
        % create a range array of 2 samples around each pk index
    ...
    for iClst=2:Nmax
        ...
        if (voidInterval>=MTI) % %USC PDPs are undilated, directly use MTI 
    ...
    [~,Allpkidcs]=findpeaks(PDP,"MinPeakDistance",4);
