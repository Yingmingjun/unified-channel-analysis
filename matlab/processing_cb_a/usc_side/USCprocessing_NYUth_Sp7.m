function [PDP_omni_over,PDP_omni,PL_omni,RMS_omniv2,AS_RX,ZS_RX,AS_TX,ZS_TX,noise_thresh]=USCprocessing_NYUth_Sp7(basePath,fileName,TR_distance,Ptx,thr,keyStr)
    %% Load file
    ...
    % The thresholded NYU files live under <repo>/data/raw_cb_a/NYU_Data_thresholded_7/7 GHz.
    P_cba = paths();
    thresholdBase = P_cba.cb_a_thresholded_7;
    ...
    if isempty(thrMatch)
        thrMatch = regexp(fileName, 'TX(?<TXID>\d+)_RX(?<RXID>\d+)', 'names');
    end
    Outdoor7 = [];
    if ~isempty(thrMatch)
        thrFile = fullfile(thresholdBase, sprintf('Data7Pack_TX%d_RX%d_Aligned.mat', ...
            str2double(thrMatch.TXID), str2double(thrMatch.RXID)));
        if exist(thrFile, 'file') == 2
            thrData = load(thrFile, 'Data7Pack_Aligned');
            if isfield(thrData, 'Data7Pack_Aligned')
                Outdoor7 = thrData.Data7Pack_Aligned;
            end
        end
    end
    if ~isempty(Outdoor7)
        % Precompute Outdoor142 angle signatures for fast matching (columns 6-9)
        OutdoorAngles = cell2mat(Outdoor7(:,6:9));
    else
        OutdoorAngles = [];
    end
        ...
        for el_tx_idx=1:N_eltx
            for az_rx_idx=1:N_azrx
                for el_rx_idx=1:N_elrx
                    ...
                    Pkthres=max(pdp_temp)-25;
                    noisethres=pow2db(mean(db2pow(pdp_temp(end-249:end))))+5;
                    thres=max(Pkthres,noisethres);
                    pdp_temp(pdp_temp<=thres)=-250;
                    PDP_dirS(:,az_tx_idx,el_tx_idx,az_rx_idx,el_rx_idx)=pdp_temp;
                    count=count+1;
                    pdp_temp=squeeze(PDP_dir(:,az_tx_idx,el_tx_idx,az_rx_idx,el_rx_idx));
                    Pkthres=max(pdp_temp)-25;
                    noisethres=pow2db(mean(db2pow(pdp_temp(end-4999:end))))+5;
                    thres=max(Pkthres,noisethres);
                    pdp_temp(pdp_temp<=thres)=-250;
                    PDP_dir(:,az_tx_idx,el_tx_idx,az_rx_idx,el_rx_idx)=pdp_temp;
                end
            end
        end
    end
    ...
    % Apply APS-based spatial lobe thresholding on RX azimuth (10 dB below RX APS peak).
    % Directions below the threshold are blanked for subsequent omni PDP and RMS-DS evaluation.
    APS_Rx_pow = sum(APDS_Rx, 1); % power vs RX azimuth (linear)
    APS_Rx_dB = pow2db(APS_Rx_pow);
    APS_Rx_th = max(APS_Rx_dB) - 10;
    badRx = APS_Rx_dB < APS_Rx_th;
    if any(badRx)
        % Blank APDS for low-APS RX azimuths (linear domain)
        PDP_temp1(:,:,badRx) = 0;
        APDS(:,:,badRx) = 0;
        APDS_Rx(:,badRx) = 0;
        % Blank time-dilated and undilated PDPs for RMS-DS consistency (dB domain)
        PDP_dirS(:,:,:,badRx,:) = -250;
        PDP_dir(:,:,:,badRx,:)  = -250;
    end
    ...
    %% Dilated PDP
    PDP_temp1_over=squeeze(sum(pdp_lin,5)); % sum over all elevations in Rx
    ...
    PL_omni=Ptx+30-Pr_omni; % PL = Ptx + G_tx(27dBi) + G_rx(27dBi) - Pr_omni
    ...
    PDP_dir_max_over=squeeze(PDP_dir(:,Az_Tx,El_Tx,Az_Rx,El_Rx)); % Given the direction we extract the corresponding PDP
    if d_LOS >= 5
    ...
    %% Angular Spread
    ang_Tx=reshape([spAngles{:,1,1,1}],4,12); % Tx_Az vector
    ang_Tx=ang_Tx(1,:);
    ang_zTx=reshape([spAngles{1,:,1,1}],4,3); % Tx_El vector
    ang_zTx=ang_zTx(2,:);
    ang_Rx=reshape([spAngles{1,1,:,1}],4,12); % Rx_Az vector
    ang_Rx=ang_Rx(3,:);
