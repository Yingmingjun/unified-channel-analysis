...
% Bias line + +/-1.96 SD dashed lines per side (matches BA_AS_Merged.m).
yline(res_n3.bias,     '-',  'Color', c_nyu, 'LineWidth', 2.0, ...
      'HandleVisibility', 'off');
yline(res_n3.loa_low,  '--', 'Color', c_nyu, 'LineWidth', 1.6, ...
      'HandleVisibility', 'off');
yline(res_n3.loa_high, '--', 'Color', c_nyu, 'LineWidth', 1.6, ...
      'HandleVisibility', 'off');
yline(res_u3.bias,     '-',  'Color', c_usc, 'LineWidth', 2.0, ...
      'HandleVisibility', 'off');
yline(res_u3.loa_low,  '--', 'Color', c_usc, 'LineWidth', 1.6, ...
      'HandleVisibility', 'off');
yline(res_u3.loa_high, '--', 'Color', c_usc, 'LineWidth', 1.6, ...
      'HandleVisibility', 'off');
yline(0, ':', 'Color', [0.5 0.5 0.5], 'LineWidth', 0.8, ...
      'HandleVisibility', 'off');

xlabel(sprintf('Mean of paired %s [%s]', metric_lbl, unit));
ylabel(sprintf('Difference (USC method - NYU method) [%s]', unit));
title(ttl);
legend('Location', 'southeast', 'FontSize', 10);

% Per-side bias/SD annotation (blue upper-left, orange upper-right).
text(0.02, 0.98, ...
