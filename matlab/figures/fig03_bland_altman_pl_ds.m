...
% --- Y-axis padding:
%   +40 %% top gives the two corner stat boxes (3 lines, ~0.10 y-norm)
%        a clear 0.08 y-norm gap above the highest line label
%        (loa_high_norm <= 0.74 in this padded axes).
%   +12 %% bottom is enough for the -1.96 SD label to sit cleanly
%        inside the axes; the legend lives OUTSIDE the axes
%        (southoutside) so it cannot block the bottom label.
drawnow;
yl = ylim(ax);  yspan = max(yl(2) - yl(1), eps);
ylim(ax, [yl(1) - 0.12*yspan, yl(2) + 0.40*yspan]);
...
% --- Legend OUTSIDE the axes (southoutside, horizontal). Placing
% the marker legend outside the plot area guarantees it cannot
% overlap any of the in-axes line labels (esp. the -1.96 SD
% labels, which previously collided with a southeast legend).
legend(ax, [h_n3, h_u3], 'Location', 'southoutside', ...
