...
%
%  PROCESSING METHOD:
%    Same as USC7GHz_Method_Comparison.m:
%    - Hann windowing + IFFT -> PDP
%    - USC noise threshold: max noise + 12 dB, DR = 22 dB cap
%    - Flip TX azimuth
%    - Omni: NYU SUM vs USC perDelayMax
...
for iLoc = 1:nFiles
    ...
    % =====================================================================
    % STEP 9: RMS Delay Spread
    % =====================================================================
    if d_LOS >= 50
        delay_vec_ns = (d + d_LOS - 10) / params.c * 1e9;
    else
    ...
    % Adjust noise threshold for gain-corrected omni PDP
    noise_thresh_dB_corrected = noise_thresh_dB - params.correction_factor_dB;
    valid_mask_NYU = PDP_omni_NYU_dB > noise_thresh_dB_corrected;
    valid_mask_USC = PDP_omni_USC_dB > noise_thresh_dB_corrected;
